...
Дла того что бы запустить Debug вурсию, следует скачать с ветки "Tests" данную сборку. Затем собрать её с помощью команды "dotnet build", и выполнить команду: dotnet run convert --input "C:\ВашаПапка\input.conf" --output "C:\ВашаПапка\output.toml"
